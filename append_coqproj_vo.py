...
def extract_qr_flags(file_v):
    try:
        ...
        # Extract lines like "-Q /path libname" or "-R /path libname"
        qr_matches = re.findall(r"-(Q|R) ([^ ]+) ([^ ]+)", result.stdout)
        flags = []
        for kind, path, name in qr_matches:
            # Normalize absolute paths to _build/default relative
...
if new_entries:
    with open(coqproject_path, "a") as f:
        for entry in new_entries:
            f.write(entry + "\n")
    print(f"✅ Appended {len(new_entries)} new entries to _CoqProject:")
    for entry in new_entries:
        print(entry)
else:
