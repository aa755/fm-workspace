...
From LF Require Export c1Basics.

(** For this [Require] command to work, Coq needs to be able to
    find a compiled version of the previous chapter ([c1Basics.v]).
    This compiled version, called [c1Basics.vo], is analogous to the
    [.class] files compiled from [.java] source files and the [.o]
    files compiled from [.c] files.

    The key binding C-c M-i compiles all the dependencies of the current file
    using the dune build system and then loads Coq for the current file
    and checks it till the cursor.
    You need to do this only if you have changed c1Basics.v and need
    this file to see your updates: the docker container setup script already compiled
    all tutorial files.
 *)
